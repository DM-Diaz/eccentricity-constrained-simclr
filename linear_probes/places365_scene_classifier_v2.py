...
"""
Places365 scene classification transfer from SimCLR-pretrained encoders.

Key features (mirrors in-domain staged classifier):
- ResNet18/ResNet50 backbone (--arch).
- Load SimCLR checkpoint and should do so robustly now (handles common key prefixes, DDP, etc.).
...
- This mirrors VGGFace-2 implementation
- !!! Sbatch params take precedence so verify against default values!!!
"""
...
def build_transforms(mode: str, img_size: int):
    ...
    if mode == "train":
        # Modest augmentation; keep comparable across runs
...
# -------------------------
# Encoder loading (robust prefix matching)
# -------------------------
@dataclass
class CkptLoadReport:
    ...
    example_unexpected: List[str]


def _extract_state_dict(obj) -> Dict[str, torch.Tensor]:
    if isinstance(obj, dict):
        for k in ("state_dict", "model_state", "model", "net", "encoder", "backbone"):
...
def load_resnet_backbone_from_simclr_checkpoint(backbone: nn.Module, ckpt_path: str, verbose: bool = True) -> CkptLoadReport:
    """
    Loads weights into a torchvision ResNet backbone (with .fc=Identity already).
    Tries common prefixes to maximize key overlap. Works with many SimCLR variants.
    """
    ckpt = torch.load(ckpt_path, map_location="cpu")
    sd = _extract_state_dict(ckpt)

    # Remove any leading 'module.' (DDP)
    sd = {(k[7:] if k.startswith("module.") else k): v for k, v in sd.items()}

    target_keys = set(backbone.state_dict().keys())

    prefixes = [
        "", "backbone.", "encoder.", "model.", "net.", "online_encoder.", "student.", "teacher.",
        "base_enc.", "fovea_enc.", "periph_enc.", "shared.",
    ]
    ...
    for pref in prefixes:
        filtered = {k[len(pref):]: v for k, v in sd.items() if k.startswith(pref)}
        overlap = len(set(filtered.keys()) & target_keys)
        if overlap > best_overlap:
            best_overlap = overlap
            best_pref = pref
            best_filtered = filtered

    # Fallback: discover prefix by locating conv1.weight
    if best_filtered is None or best_overlap < 30:
        candidates = []
        for k in sd.keys():
            if k.endswith("conv1.weight"):
                candidates.append(k[:-len("conv1.weight")])
        for pref in candidates[:50]:
            filtered = {k[len(pref):]: v for k, v in sd.items() if k.startswith(pref)}
            overlap = len(set(filtered.keys()) & target_keys)
            if overlap > best_overlap:
                best_overlap = overlap
    ...
    if best_filtered is None or best_overlap < 30:
        raise RuntimeError(f"Could not match checkpoint keys to backbone. best_overlap={best_overlap}")

    missing, unexpected = backbone.load_state_dict(best_filtered, strict=False)

    report = CkptLoadReport(
        chosen_prefix=best_pref,
        overlap=int(best_overlap),
        target_keys=len(target_keys),
        missing=len(missing),
        unexpected=len(unexpected),
        example_missing=[str(x) for x in list(missing)[:10]],
        example_unexpected=[str(x) for x in list(unexpected)[:10]],
    )

    if verbose:
        print(f"[LOAD] ckpt={ckpt_path}")
        print(f"[LOAD] chosen_prefix='{report.chosen_prefix}' overlap={report.overlap}/{report.target_keys}")
        print(f"[LOAD] missing={report.missing} unexpected={report.unexpected}")
        if report.example_missing:
            print(f"[LOAD] example missing: {report.example_missing[:5]}")
        if report.example_unexpected:
            print(f"[LOAD] example unexpected: {report.example_unexpected[:5]}")
...
def main():
    ...
    ap.add_argument("--data-root", default="/lab_data/tarrlab/common/datasets/Places365",
                    help="Folder in ImageFolder format: data_root/<class_name>/*.jpg")
    ...
    # Convenience aliases (kept for sbatch compatibility)
    ap.add_argument("--lr", type=float, default=None,
                    help="Alias for stage LR: overrides --probe-lr if stage=probe; overrides --finetune-lr if stage includes finetune.")
    ...
    ap.add_argument("--pin-memory", action="store_true",
                    help="Alias (default pin_memory=True). If set, overrides --no-pin-memory.")
    ap.add_argument("--no-persistent-workers", action="store_true")
    ap.add_argument("--persistent-workers", action="store_true",
                    help="Alias (default persistent_workers=True). If set, overrides --no-persistent-workers.")
    ...
    # reconcile convenience aliases
    if getattr(args, "pin_memory", False):
        args.no_pin_memory = False
    if getattr(args, "persistent_workers", False):
        args.no_persistent_workers = False
    if args.grad_accum < 1:
        ap.error("--grad-accum must be >= 1")

    # This is a lot of vars, make sure all correct (fragile will need to update at some point)
    cfg = Config(
    ...
    if args.lr is not None:
        if cfg.stage == "probe":
            cfg.probe_lr = float(args.lr)
        elif cfg.stage == "finetune":
            cfg.finetune_lr = float(args.lr)
        else:
            # stage=="both": keep probe_lr as specified, but override finetune_lr
            cfg.finetune_lr = float(args.lr)
