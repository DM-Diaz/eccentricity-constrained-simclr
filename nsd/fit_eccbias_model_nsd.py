...
def fit_model(args):
    ...
    features_file_list = get_features_filename(args)
    ...
    # print('skip z-score')
    # f_trn, f_val, f_nest = model_fitting_utils.split_feats(f, trn_inds, val_inds, nest_inds)
    ...
    dict2save = {'subject': args.subject, \
                 ...
                 'r2': r2, \
                 'corr': corr, \
                 'best_lambda_inds': best_lambda_inds, \
                    }

    save_folder = os.path.join(save_fits_folder, args.model_name)
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    fn2save = os.path.join(save_folder, 'NSD_S%d_%s_%s.npy'%(args.subject, args.model_name, args.model_layer))
...
def get_features_filename(args):
    ...
    if args.model_name=='RN50-CLIP-vision':
        feat_path = os.path.join(features_folder, args.model_name)
    ...
    elif 'resnet18' in args.model_name:
        feat_path = os.path.join(features_folder, 'eccbias',args.model_name)
